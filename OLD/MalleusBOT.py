...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- SPOSTATO QUI: Estrai full_name, username e user_identifier subito ---
    full_name = user.full_name # Nome completo dell'utente (nome + cognome)
    username = user.username # Username dell'utente (es. @username)
    ...
    if user_status == "administrator" or user_status == "creator":
        print(f"DEBUG: Utente {full_name} (ID: {user.id}) è un amministratore/creatore. Esentato dalle restrizioni di orario e lunghezza.")
    ...
    # Queste righe non sono più necessarie qui, perché full_name e user_identifier sono già stati definiti
    # full_name = user.full_name
    # username = user.username
    # user_identifier = f"{full_name} {username}" if username else full_name

    if not user_identifier: # Questo controllo è ancora utile per casi limite
